...
#define VIDEO_WIDTH 640 // smaller will be faster
#define VIDEO_HEIGHT 360 // smaller will be faster
int key_pressed;
...
static void encoder_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer) {
    ...
    // userdata->image = cvDecodeImage(buf, CV_LOAD_IMAGE_COLOR);

    memcpy(userdata->image->imageData, buffer->data, userdata->video_width * userdata->video_height); // read Y
    // printf("%d\n", buffer->length);
    // NOT WORKING
    memcpy(userdata->imageU->imageData, buffer->data+w*h, w*h); // read U
    // printf("%x\n", (int)(buffer->data));
    // printf("%x\n", (int)(buffer->data+w*h));
    // printf("%x\n", (int)(buffer->data+w*h+w*h4));
    // memcpy(userdata->imageVB->imageData,buffer->data+w*h+w*h,w*h); // read v
...
int main(int argc, char** argv) {
    ...
    // // *** MODIFICATION Create encoder component
    ...
    userdata.image = cvCreateImage(cvSize(userdata.video_width, userdata.video_height), IPL_DEPTH_8U, 1); // Y
    ...
    userdata.imageU = cvCreateImage(cvSize(userdata.video_width / 2, userdata.video_height / 2), IPL_DEPTH_8U, 1); //U
    userdata.imageV = cvCreateImage(cvSize(userdata.video_width / 2, userdata.video_height / 2), IPL_DEPTH_8U, 1); //V 
    ...
    camera_video_port->buffer_size = userdata.preview_width * userdata.preview_height * 16 / 8;
